...
function buildOptimizedImageUrl(src: string, width: number, quality = 50): string {
  ...
  if (
    convertedSrc.startsWith("/") ||
    convertedSrc.startsWith("data:") ||
    convertedSrc.startsWith("blob:")
  ) {
    return convertedSrc;
  }

  // Proxy all remote assets through /api/image so we can resize/re-encode/cache.
  if (!/^https?:\/\//i.test(convertedSrc)) {
    return convertedSrc;
  }
